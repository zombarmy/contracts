...
contract Treasury is ContractGuard {
    ...
    uint256 public mintingFactorForPayingDebt; // print extra ZOMB during debt phase
    ...
    address public bondFund;
    uint256 public bondFundSharedPercent;
    ...
    event BondFundFunded(uint256 timestamp, uint256 seigniorage);
    ...
    function getTombPrice() public view returns (uint256 tombPrice) {
        try IOracle(tombOracle).consult(tomb, 1e18) returns (uint144 price) {
            ...
            revert("Treasury: failed to consult ZOMB price from the oracle");
        }
    }
    ...
    function getBondDiscountRate() public view returns (uint256 _rate) {
        ...
        if (_tombPrice <= tombPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _bondAmount = tombPriceOne.mul(1e18).div(_tombPrice); // to burn 1 ZOMB
    ...
    function initialize(
        ...
        maxExpansionTiers = [400, 300, 200, 150, 100, 75, 50];

        maxSupplyExpansionPercent = 400; // Upto 4.0% supply for expansion

        bondDepletionFloorPercent = 10000; // 100% of Bond supply for depletion floor
        seigniorageExpansionFloorPercent = 3500; // At least 35% of expansion reserved for masonry
        maxSupplyContractionPercent = 300; // Upto 3.0% supply for contraction (to burn ZOMB and mint ZBOND)
    ...
    function setExtraFunds(
        ...
        bondFund = _bondFund;
        bondFundSharedPercent = _bondFundSharedPercent;
    }
    ...
    function buyBonds(uint256 _tombAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        require(tombPrice == targetPrice, "Treasury: ZOMB price moved");
        require(
    ...
    function _sendToMasonry(uint256 _amount) internal {
        ...
        _amount = _amount.sub(_daoFundSharedAmount).sub(_devFundSharedAmount).sub(_bondFundSharedAmount);
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (previousEpochTombPrice > tombPriceCeiling) {
            // Expansion ($ZOMB Price > 1 $FTM): there is some seigniorage to be allocated
